...
$(function () {
	//页面初始化
	listPageInit("singletree.shtml?action=toList");
	//新增和修改按钮初始化
	mtbAddModiInit();
    ...
    if(null == selRowId || "" == selRowId){
		  btnDisable(["start","modify","delete","stop","pageCustomized"]);
 	 	  return;
 	 } 
...
function listDelConfirm(){
	...
	var selRowId = $("#selRowId").val();
	 $.ajax({
	 	url: "singletree.shtml?action=delete&JGXXID="+selRowId,
		type:"POST",
		complete: function(xhr){
			...
			if(jsonResult.success===true){
				...
				window.location="singletree.shtml?action=toList&JGXXID="+selNodesId;
			}else{
...
function listStopConfirm(){
	...
	var selRowId = $("#selRowId").val();
	 $.ajax({
		url: "singletree.shtml?action=changeStateTy&JGXXID="+utf8(selRowId),
		type:"POST",
		dataType:"text",
		complete: function(xhr){
			eval("jsonResult = "+xhr.responseText);
			if(jsonResult.success===true){
				...
				window.location.href="singletree.shtml?action=toList&JGXXID="+parent.leftcontent.getSelNodesId();
			}else{
...
function listStartConfirm(){
	...
	var selRowId = $("#selRowId").val();
    $.ajax({
	 url: "singletree.shtml?action=changeStateQy&JGXXID="+utf8(selRowId),
	 type:"POST",
 	 dataType:"text",
	 complete: function(xhr){
		eval("jsonResult = "+xhr.responseText);

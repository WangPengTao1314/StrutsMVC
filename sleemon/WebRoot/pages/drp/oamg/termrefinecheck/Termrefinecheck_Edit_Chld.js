...
function allSave(){
	...
	$.ajax({
		url: "termrefinecheck.shtml?action=edit",
		type:"POST",
		dataType:"text",
		data:{"parentJsonData":parentData,"childJsonData":childData,"TERM_REFINE_CHECK_ID":selRowId,"chkLen":chkLen},
		complete: function (xhr){
			eval("jsonResult = "+xhr.responseText);
			if(jsonResult.success===true){
				saveSuccess("保存成功","termrefinecheck.shtml?action=toFrame");
			}else{
...
function childSave(){
	...
	$.ajax({
		url: "termrefinecheck.shtml?action=childEdit",
		type:"POST",
		dataType:"text",
		data:{"childJsonData":jsonData,"TERM_REFINE_CHECK_ID":selRowId},
		complete: function(xhr){
			eval("jsonResult = "+xhr.responseText);
			if(jsonResult.success===true){
...
function gobacknew()
{
   newGoBack("termrefinecheck.shtml?action=toFrame");
}
...
function addRow(arrs){
	...
	$("#jsontb tr:last-child")
		    .append("<td nowrap align='center'><input type='checkbox' style='height:12px;valign:middle' json='TERM_REFINE_CHECK_DTL_ID' name='TERM_REFINE_CHECK_DTL_ID' value='"+arrs[0]+"'/></td>")
             .append('<input type="hidden" json="TERM_REFINE_CHECK_ID" id="TERM_REFINE_CHECK_ID'+rownum+'" name="TERM_REFINE_CHECK_ID"  label="门店精致化检查ID" maxlength="32" value="'+arrs[1]+'"/>')
            ...
            .append('<td nowrap align="center"><input  json="CHECK_REMARK" id="CHECK_REMARK'+rownum+'" name="CHECK_REMARK"  autocheck="true" label="扣分原因"  type="text"   inputtype="string"       mustinput="true"     maxlength="250"  value="'+arrs[6]+'"/>&nbsp;</td>')
            .append('<td nowrap align="center"><input  json="CHECK_SCORE" id="CHECK_SCORE'+rownum+'" name="CHECK_SCORE"  autocheck="true" label="检查结果得分" onkeyup="countScore('+rownum+')"  type="text"   inputtype="float"       mustinput="true" valueType="8,2"    maxlength="22"  value="'+arrs[7]+'"/>&nbsp;</td>')

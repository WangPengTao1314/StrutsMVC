...
$(function() {
	...
	//主从及主从从列表页面通用加载方法
	listPageInit("channscoremkm.shtml?action=toList&module=" + module);
	//新增和修改按钮初始化
	addModiToEditFrameInit("channscoremkm.shtml?action=toEditFrame");
 ...
 function toFlow(i) {
    ...
    document.affirm.sourceURI.value=ctxPath+"/channscoremkm.shtml?action=toList"+document.getElementById("paramUrl").value ;
	switch (Number(i)) {
	  case 1:
		affirmEvent(0);
		break;//提交
...
function listDelConfirm(){
	...
	//alert("selRowId===="+selRowId);
	 $.ajax({
	 	url: "channscoremkm.shtml?action=delete&CHANN_SCORE_MKM_ID="+selRowId,
		type:"POST",

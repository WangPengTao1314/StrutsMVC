...
$(function() {
	...
	whBtnHidden(["audit"]);
	//审核页面需要隐藏的按钮
	shBtnHidden(["add","delete","modifyT", "commit", "revoke"]);
	...
	if (selRowId == null || '' == selRowId) {
		btnDisable( [ "start", "modifyT", "delete", "stop" ]);
		return;
	}
 ...
 function toFlow(i) {
    ...
    document.affirm.sourceURI.value=ctxPath+"/advreit.shtml?action=toList"+document.getElementById("paramUrl").value ;
	switch (Number(i)) {
	  case 1:
		affirmEvent(0);
		break;//提交
...
function listDelConfirm(){
	...
	var selRowId = $("#selRowId").val();
	 $.ajax({
	 	url: "advreit.shtml?action=delete&ERP_ADV_REIT_ID="+selRowId,
		type:"POST",
		complete: function(xhr){
			eval("jsonResult = "+xhr.responseText);
			if(jsonResult.success===true){

...
$(function() {
	...
	//主从及主从从列表页面通用加载方法
	listPageInit("terminalchange.shtml?action=toList");
 ...
 function orpenWindow(TERM_NO){
	window.showModalDialog("terminalchange.shtml?action=toShowHistory&TERM_NO="+TERM_NO,window,"dialogwidth=900px; dialogheight=500px; status=no");
 }
 ...
 function toAudit(){
     var selRowId = $("#selRowId").val();
     $.ajax({
		url: "terminalchange.shtml?action=toAudit&TERMINAL_CHANGE_ID="+selRowId,
		type:"POST",
		dataType:"text",
		data:{"TERMINAL_CHANGE_ID":selRowId},
		complete: function (xhr){
		}
	 }); 
 }
 ...
 function toFlow(i) {
    ...
    document.affirm.sourceURI.value=ctxPath+"/terminalchange.shtml?action=toList"+document.getElementById("paramUrl").value ;
	switch (Number(i)) {
	  case 1:
		affirmEvent(0);
		break;//提交
...
function listDelConfirm(){
	...
	var selRowId = $("#selRowId").val();
	 $.ajax({
	 	url: "terminalchange.shtml?action=delete&TERMINAL_CHANGE_ID="+selRowId,
		type:"POST",
		complete: function(xhr){
			eval("jsonResult = "+xhr.responseText);
			if(jsonResult.success===true){

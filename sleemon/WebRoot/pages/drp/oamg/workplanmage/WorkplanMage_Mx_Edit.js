...
function childSave(){
   	...
   	var PLAN_YEAR = parent.topcontent.document.getElementById("PLAN_YEAR").value;
	if(PLAN_YEAR ==""){
	    parent.showErrorMsg("请选择计划年份");
	    return;
	}
	
	var PLAN_MONTH = parent.topcontent.document.getElementById("PLAN_MONTH").value;
	if(PLAN_MONTH ==""){
	...
	$.ajax({
		...
		complete: function(xhr){
			...
			if(jsonResult.success===true){
				...
				//alert(parent.topcontent.document.getElementById("TOTAL_UP_REPORT_NUM"));
...
function addRow(arrs){
	 ...
	 if(parent.topcontent.document.getElementById("TOTAL_UP_REPORT_NUM")!=null){
	  parent.topcontent.document.getElementById("TOTAL_UP_REPORT_NUM").value = rownum;
	 }
}

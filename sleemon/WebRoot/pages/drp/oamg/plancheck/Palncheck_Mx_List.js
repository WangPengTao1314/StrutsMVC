...
$(function(){
	...
	if(parent.topcontent.document.getElementById("state"+selRowId) !=null){
	...
	$("#edit").click(function(){
		...
		$("#CHANN_CHECK_PLAN_DTL_IDS").val(ids);
		$("#form1").attr("action","plancheck.shtml?action=toChildEdit");
		$("#form1").submit();
	});
...
//查找当前是否有选中的记录
	...
	ids = ids.substr(0,ids.length-1);
	$.ajax({
		url: "plancheck.shtml?action=childDelete",
		type:"POST",
		dataType:"text",
		data:{"CHANN_CHECK_PLAN_DTL_IDs":ids,"CHANN_CHECK_PLAN_ID":selRowId},

...
function allSave(){
	...
	$.ajax({
		url: "storeinnotice.shtml?action=edit",
		type:"POST",
		dataType:"text",
		...
		complete: function (xhr){
			...
			if(jsonResult.success===true){
				saveSuccess("保存成功","storeinnotice.shtml?action=toFrame");
			}else{
...
function childSave(){
	...
	$.ajax({
		url: "storeinnotice.shtml?action=childEdit",
		type:"POST",
		dataType:"text",
...
//查找当前是否有选中的记录
	...
	$.ajax({
		url: "storeinnotice.shtml?action=childDelete",
		type:"POST",
		dataType:"text",
...
//返回
function gobacknew(){
   newGoBack("storeinnotice.shtml?action=toFrame");
}
...
function addRow(arrs){
	...
	$("#jsontb tr:last-child")
            ...
            .append('<td nowrap align="left"><input  json="STOREIN_STORE_ID" id="STOREIN_STORE_ID'+rownum+'" name="STOREIN_STORE_ID'+rownum+'" label="入库库房ID"  type="hidden" value="'+arrs[1]+'"/><input  json="STOREIN_STORE_NO" id="STOREIN_STORE_NO'+rownum+'" size="12"  name="STOREIN_STORE_NO'+rownum+'"  autocheck="true" label="入库库房编号" size="6"  type="text"   inputtype="string"   readonly    mustinput="true"  maxlength="30"  value="'+arrs[2]+'"/>&nbsp;<img align="absmiddle" name="selJGXX" style="cursor:hand" src="/sleemon/styles/newTheme/images/plus/select.gif" onclick="javascript:selStore('+rownum+')"/>&nbsp;</td>')
            .append('<td nowrap align="left"><input  json="STOREIN_STORE_NAME" id="STOREIN_STORE_NAME'+rownum+'" name="STOREIN_STORE_NAME'+rownum+'"  autocheck="true" label="入库库房名称"  type="text"   inputtype="string"     mustinput="true" size="12"  readonly   maxlength="100"  value="'+arrs[3]+'"/>&nbsp;</td>')

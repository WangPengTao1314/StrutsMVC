...
$(function(){
	...
	//主从及主从从列表页面通用加载方法
	listPageInit("storeinnotice.shtml?action=toList&module=" + module);
	//新增和修改按钮初始化
	addModiToEditFrameInit("storeinnotice.shtml?action=toEditFrame");
	//删除监听.参数1：删除action，参数2：删除主键Id
	mtbDelListener("storeinnotice.shtml?action=delete", "STOREIN_NOTICE_ID");
    $("#commit").click(function () {
		...
		showWaitPanel();
		$.ajax({
			url: "storeinnotice.shtml?action=toCommit",
			type:"POST",
			dataType:"text",
 ...
 function toFlow(i) {
    ...
    document.affirm.sourceURI.value=ctxPath+"/storeinnotice.shtml?action=toList"+document.getElementById("paramUrl").value ;
	switch (Number(i)) {
	  case 1:
		affirmEvent(0);
		break;//提交

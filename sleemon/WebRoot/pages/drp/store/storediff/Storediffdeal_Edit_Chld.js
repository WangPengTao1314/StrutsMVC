...
function allSave(){
	...
	$.ajax({
		url: "storediffdeal.shtml?action=edit",
		type:"POST",
		dataType:"text",
		...
		complete: function (xhr){
			...
			if(jsonResult.success===true){
				saveSuccess("保存成功","storediffdeal.shtml?action=toFrame");
			}else{
...
function childSave(){
	...
	$.ajax({
		url: "storediff.shtml?action=dealEdit",
		type:"POST",
		dataType:"text",
...
//查找当前是否有选中的记录
	...
	$.ajax({
		url: "storediffdeal.shtml?action=childDelete",
		type:"POST",
		dataType:"text",
...
function gobacknew()
{
   newGoBack("storediff.shtml?action=toFrame");
}
...
function addRow(arrs){
	...
	$("#jsontb tr:last-child")
            ...
            .append('<td nowrap align="left"><select  json="DIFF_RSON" id="DIFF_RSON'+rownum+'" name="DIFF_RSON'+rownum+'"   onchange="changeDealway('+rownum+',1)"  autocheck="true" label="差异原因"      mustinput="true"     maxlength="30"  value="'+arrs[12]+'"></select></td>')
            .append('<td nowrap align="left"><select  json="DEAL_WAY" id="DEAL_WAY'+rownum+'" name="DEAL_WAY'+rownum+'"    autocheck="true" label="处理方式"   inputtype="string"     mustinput="true"     maxlength="30"  value="'+arrs[13]+'"></select></td>')
            ...
            .append('<td nowrap align="left"><input  json="DIFF_ATT" id="DIFF_ATT'+rownum+'" name="DIFF_ATT'+rownum+'"  autocheck="true" label="差异附件"  type="text"   inputtype="string"     mustinput="false"     maxlength="100"  value="'+arrs[15]+'"/>&nbsp;</td>')
	...
	changeDealway(rownum,arrs[12]);	
}

function changeDealway(rowNum,oval){
	var v ;
	if(null == oval || "" == oval||"1"==oval){
		v= $("#DIFF_RSON"+rowNum).val();
		v = $.trim(v);
	}else{
		v = oval
	}

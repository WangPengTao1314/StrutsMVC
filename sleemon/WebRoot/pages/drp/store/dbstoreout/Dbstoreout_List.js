...
$(function(){
	...
	//主从及主从从列表页面通用加载方法
	listPageInit("dbstoreout.shtml?action=toList&module=" + module+"&BILL_TYPE="+billType);
	$("#done").click(function(){
	  doneCommit();
	});
	setSelOperateEx();
...
function listCommitConfirm(){
    ...
    $.ajax({
	 url: "dbstoreout.shtml?action=toCommit&STOREOUT_ID="+utf8(selRowId)+"&BILL_TYPE="+billType,
	 type:"POST",
 	 dataType:"text",

...
$(function(){
	...
	$("#edit").click(function(){
		...
		$("#form1").attr("action","sporadicstoreout.shtml?action=toChildEdit&BILL_TYPE="+billType);
		$("#form1").submit();
	});
	...
	$("#scan").click(function(){
		...
		window.open('sporadicstoreout.shtml?action=toEditScanDtl&STOREOUT_ID='+storeoutId+'&BILL_TYPE='+billType,'货品扫码明细','height=400, width=1100, top=200, left=200, toolbar=no, menubar=no, scrollbars=yes, resizable=no,location=no, status=no');
	});
...
//查找当前是否有选中的记录
	...
	$.ajax({
		url: "sporadicstoreout.shtml?action=childDelete",
		type:"POST",
		dataType:"text",
...
function showStoge(STOREOUT_DTL_ID){
	...
	window.showModalDialog('sporadicstoreout.shtml?action=storgChildList&STOREOUT_DTL_IDS='+ STOREOUT_DTL_IDS+"&BILL_TYPE="+billType+"&STOREOUT_ID="+STOREOUT_ID,window,"dialogWidth=900px;dialogHeight=500px;depended=no");

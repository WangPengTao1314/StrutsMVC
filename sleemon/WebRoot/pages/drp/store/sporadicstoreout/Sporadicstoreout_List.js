...
$(function(){
    ...
    //主从及主从从列表页面通用加载方法
	listPageInit("sporadicstoreout.shtml?action=toList&module=" + module+"&BILL_TYPE="+billType);
	$("#done").click(function(){
	  doneCommit();
	});
	setSelOperateEx();
...
function modifyRemark(){
	...
	closePage();
	$.ajax({
	 	url: "sporadicstoreout.shtml?action=modifyRemark",
		type:"POST",
		data:{"STOREOUT_ID":selRowId,"REMARK":REMARK,"STOREOUT_TIME":STOREOUT_TIME},
		complete: function(xhr){
			eval("jsonResult = "+xhr.responseText);
			if(jsonResult.success===true){
...
function listCommitConfirm(){
	...
	var selRowId = $("#selRowId").val();
    $.ajax({
	 url: "sporadicstoreout.shtml?action=toCommit&STOREOUT_ID="+utf8(selRowId)+"&BILL_TYPE="+billType,
	 type:"POST",
 	 dataType:"text",
	 complete: function(xhr){
		eval("jsonResult = "+xhr.responseText);
...
//	$("#printIframe").attr("src","sporadicstoreout.shtml?action=printInfo&STOREOUT_ID="+selRowId);
	window.open("sporadicstoreout.shtml?action=sporadicPrintInfo&STOREOUT_ID="+selRowId,'报表查看','height=600, width=800, top=100, left=100, toolbar=yes, menubar=yes, scrollbars=yes, resizable=yes,location=yes, status=yes');
}
...
function closeDocument(){
	var selRowId = $("#selRowId").val();
	$.ajax({
	 url: "sporadicstoreout.shtml?action=closeDocument",
	 type:"POST",
 	 dataType:"text",
 	 data:{"STOREOUT_ID":selRowId,"BILL_TYPE":"零星出库"},
	 complete: function(xhr){
		eval("jsonResult = "+xhr.responseText);
		if(jsonResult.success===true){

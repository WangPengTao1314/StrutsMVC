...
$(function(){
    ...
    //主从及主从从列表页面通用加载方法
	listPageInit("repairstoreout.shtml?action=toList&module=" + module+"&BILL_TYPE="+billType);
	$("#done").click(function(){
	  doneCommit();
	});
	setSelOperateEx();
...
function checkSN(){
	...
	tabObj.each(function(){
		var NOTICE_NUM =  $.trim($(this).find("td[name='NOTICE_NUM']").text());
...
function listCommitConfirm(){
    ...
    $.ajax({
	 url: "repairstoreout.shtml?action=toCommit&STOREOUT_ID="+utf8(selRowId)+"&BILL_TYPE="+billType,
	 type:"POST",
 	 dataType:"text",

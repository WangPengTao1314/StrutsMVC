...
function allSave(){
	...
	$.ajax({
		url: "repairstoreout.shtml?action=edit",
		type:"POST",
		dataType:"text",
		...
		complete: function (xhr){
			...
			if(jsonResult.success===true){
				saveSuccess("保存成功","repairstoreout.shtml?action=toFrame");
			}else{
...
function childSave(){
	...
	$.ajax({
		url: "repairstoreout.shtml?action=childEdit&BILL_TYPE="+billType,
		type:"POST",
		dataType:"text",
...
//返回
function gobacknew(){
   newGoBack("repairstoreout.shtml?action=toFrame");
}
...
function editGrendChild(storeOutDtlId,storeOutId,rownum) {
	...
	var billType=parent.window.document.getElementById("BILL_TYPE").value;
	var strRealNum = window.showModalDialog('repairstoreout.shtml?action=toStorgChildEdit&STOREOUT_DTL_ID='+ storeOutDtlId+"&STOREOUT_ID="+storeOutId+"&BILL_TYPE="+billType+"&STOREOUT_STORE_ID="+storeId,window,"dialogWidth=900px;dialogHeight=500px;depended=no");

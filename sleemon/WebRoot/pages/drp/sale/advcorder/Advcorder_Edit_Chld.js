...
function allSave(){
	...
	var oldSaleDate=parent.topcontent.getOldSaleDate();
	var updateFlag=parent.topcontent.getUpdateFlag();
	//获取json数据
	...
	$.ajax({
		url: "advcorder.shtml?action=edit",
		type:"POST",
		dataType:"text",
		data:{"parentJsonData":parentData,"childJsonData":childData,"ADVC_ORDER_ID":selRowId,"oldSaleDate":oldSaleDate,"updateFlag":updateFlag },
		complete: function (xhr){
			eval("jsonResult = "+xhr.responseText);
			if(jsonResult.success===true){
				saveSuccess("保存成功","advcorder.shtml?action=toFrame");
			}else{
...
function childSave(){
	...
	$.ajax({
		url: "advcorder.shtml?action=childEdit",
		type:"POST",
		dataType:"text",
...
//查找当前是否有选中的记录
	...
	$.ajax({
		url: "advcorder.shtml?action=childDelete",
		type:"POST",
		dataType:"text",
...
function gobacknew()
{
   newGoBack("advcorder.shtml?action=toFrame");
}
...
function addRow(arrs){
	...
	if(arrs[9]==null||arrs[9]==""||(arrs[9]=="0"&&""==arrs[0])){
		arrs[9]="1";
	}
	...
	$("#jsontb tr:last-child")
		    ...
		    .append('<td nowrap align="left" id="PRD_NOTd'+rownum+'"><input  json="PRD_NO" id="PRD_NO'+rownum+'" onblur="closeDiv()" onkeyup="seachBoxPRD(\'PRD_NO\','+rownum+')"  name="PRD_NO"    autocheck="true" label="货品编号" size="8"  type="text"  mustinput="true"   value="'+arrs[1]+'"/>&nbsp;' +
            "<img name='imgTab' id='img"+rownum+"' align='absmiddle' style='cursor: hand' src='/sleemon/styles/newTheme/images/plus/select.gif' onClick='selcPrd("+rownum+")'/></td>")
            ...
            .append('<td nowrap align="left" id="PRD_NAMETd'+rownum+'"><input  json="PRD_NAME" id="PRD_NAME'+rownum+'" onblur="closeDiv()" onkeyup="seachBoxPRD(\'PRD_NAME\','+rownum+')" name="PRD_NAME"  autocheck="true" label="货品名称"  type="text"    mustinput="true"     maxlength="100"  value="'+arrs[3]+'"/>&nbsp;</td>')
            .append('<td nowrap align="left"><select  json="PRD_TYPE" id="PRD_TYPE'+rownum+'" name="PRD_TYPE" mustinput="true"  autocheck="true" label="货品类型" inputtype="string"  onchange="countDISCOUNTMoney();"  value="'+arrs[17]+'"></select>&nbsp;</td>')
            ...
            .append('<td nowrap align="center" id="spclTd'+rownum+'"><font id="SPECIAL_FLAG'+rownum+'">'+SPECIAL_FLAG+'</font> <input class="btn"  value="编辑" onclick="url('+rownum+')"  type="button" />&nbsp;</td>')
            ...
            .append('<td nowrap align="center" style="display: none;"><input  json="IS_FREEZE_FLAG" id="IS_FREEZE_FLAG'+rownum+'" name="IS_FREEZE_FLAG"  label="是否冻结" onclick="freezeClick('+rownum+')"     type="checkbox" value="0"/>&nbsp;</td>')
            .append('<td nowrap align="left" style="display: none;"><input  json="FREEZE_NUM" id="FREEZE_NUM'+rownum+'" name="FREEZE_NUM'+rownum+'"  autocheck="true" label="冻结数量"  type="text" readonly size="3" inputtype="int"  mustinput="true"     maxlength="22"  value="'+arrs[22]+'" />&nbsp;</td>')
            .append('<td nowrap align="left" style="display: none;><input  json="FREEZE_STORE_NAME" id="FREEZE_STORE_NAME'+rownum+'" name="FREEZE_STORE_NAME"  inputtype="string"  autocheck="true" label="冻结库房名称"  type="text"    READONLY  value="'+arrs[21]+'"/>&nbsp;' +
            "<img name='imgTab' align='absmiddle' style='cursor: hand' src='/sleemon/styles/newTheme/images/plus/select.gif' onClick='selStore("+rownum+")'/></td>")
            .append('<input  json="FREEZE_STORE_ID" id="FREEZE_STORE_ID'+rownum+'" name="FREEZE_STORE_ID"  label="冻结库房ID" type="hidden" value="'+arrs[19]+'"/>')
            .append('<input  json="FREEZE_STORE_NO" id="FREEZE_STORE_NO'+rownum+'" name="FREEZE_STORE_NO"  label="冻结库房NO" type="hidden" value="'+arrs[20]+'"/>')
            ...
            .append('<input  json="ADVC_ORDER_ID" id="ADVC_ORDER_ID'+rownum+'" name="ADVC_ORDER_ID"  label="预订单ID" type="hidden" value="'+arrs[16]+'"/>')
            .append('<input  json="SPCL_TECH_ID" id="SPCL_TECH_ID'+rownum+'" name="SPCL_TECH_ID"  label="特殊工艺单ID" type="hidden" value="'+arrs[15]+'"/>')
            .append('<input  json="SPCL_TECH_FLAG" id="SPCL_TECH_FLAG'+rownum+'" name="SPCL_TECH_FLAG"  label="特殊工艺标记" type="hidden" value="'+arrs[24]+'"/>')
              ;
...
function upInput(id){
	$("img[name='imgTab']").hide();
	$("#PRD_TYPE"+id).attr("disabled","disabled");
	$("#PRD_NO"+id).attr("disabled","disabled");
	$("#PRD_NAME"+id).attr("disabled","disabled");
	var SPCL_TECH_FLAG=$("#SPCL_TECH_FLAG"+id).val();
	if(SPCL_TECH_FLAG>1){
		$("#spclTd"+id).html("<font style='color: red'>有</font><input class='btn'  value='查看' onclick='urlShow("+rownum+")'  type='button' /></span>");
	}else{
		$("#spclTd"+id).html("无");
	}
	$("#PRICE"+id).attr("readonly","readonly");
	$("#DECT_RATE"+id).attr("readonly","readonly");
	$("#DECT_PRICE"+id).attr("readonly","readonly");
	$("#PAYABLE_AMOUNT"+id).attr("readonly","readonly");
	
	$("#ORDER_NUM"+id).attr("readonly","readonly");
	$("#IS_FREEZE_FLAG"+id).attr("disabled","disabled");
}
...
//					"p.PRD_COLOR,"+
//					"p.VOLUME, "+
//					"sum(NVL(a.STORE_NUM,0)-NVL(a.FREEZE_NUM,0))NUM"+
//			" from BASE_PRODUCT p "+
//			" left join DRP_RETAL_PRICE u on u.PRD_ID=p.PRD_ID and u.DEL_FLAG = 0 and u.LEDGER_ID='"+LEDGER_ID+"'  "+
//			" left join DRP_STORE_ACCT a on a.PRD_ID=p.PRD_ID and a.LEDGER_ID='"+LEDGER_ID+"'"+
//			" where p.DEL_FLAG=0 and p.STATE in ('启用','停用') and p.FINAL_NODE_FLAG=1 and NVL(p.PRD_SUIT_FLAG,0)=0 "+
//			" and (p.COMM_FLAG=1 or (p.COMM_FLAG=0 and p.LEDGER_ID='"+LEDGER_ID+"'))   and p.IS_NO_STAND_FLAG=0 "+
//			" group by p.PRD_ID,p.PRD_NO,p.PRD_NAME,p.PRD_SPEC,p.STD_UNIT,p.BRAND,(case when u.RETAL_PRICE_ID is null then p.FACT_PRICE else u.FACT_PRICE end),p.PRD_COLOR,p.VOLUME )";
	     var obj=selCommon("BS_124", true, "PRD_ID"+rownum, "PRD_ID", "forms[0]","PRD_ID"+rownum+",PRD_NO"+rownum+",PRD_NAME"+rownum+",PRD_SPEC"+rownum+",PRD_COLOR"+rownum+",BRAND"+rownum+",STD_UNIT"+rownum+",PRICE"+rownum, 
	    	 				"PRD_ID,PRD_NO,PRD_NAME,PRD_SPEC,PRD_COLOR,BRAND,STD_UNIT,FACT_PRICE", "selectPro");
    ...
    function selStore(rownum){
    	 ...
    	 var sql="(" +
    	 			"select b.STORE_NO," +
		    	 		...
		    	 		"),0) NUM" +
	    	 		" from DRP_STORE b" +
	    	 		" where LEDGER_ID ='"+CHANN_ID+"'  and b.DEL_FLAG = 0)temp";
...
//		}
	...
	//下帧改变应收金额时算出优惠金额赋值到上贞优惠金额
	function countDISCOUNTMoney(){
		var inputs = $("select[name='PRD_TYPE']");
		var total=0;
		inputs.each(function(){
			var v = $(this).val();
			if("赠品"==v){
				var DISCOUNT_AMOUNT=$(this).parent().parent().find("input[name='PAYABLE_AMOUNT']").val();
				DISCOUNT_AMOUNT=isNaN(DISCOUNT_AMOUNT)?0:parseFloat(DISCOUNT_AMOUNT);
				total = Math.round((isNaN(total+DISCOUNT_AMOUNT)?0:total+DISCOUNT_AMOUNT)*100)/100;
			}
		});
		parent.topcontent.setDISCOUNT_AMOUNT(total);
	}
	function url(rownum){
		var PRD_ID=$("#PRD_ID"+rownum).val();
		if(""==PRD_ID||null==PRD_ID){
...
//				$("#ORDER_RECV_DATE"+id).val("");
	...
	function checkNum(){
		...
		checkBox.each(function(){
			...
			var PRD_NO=$(this).parent().parent().find("input[name='PRD_NO']");//货品编号
			var PRD_NAME=$(this).parent().parent().find("input[name='PRD_NAME']");//货品名称
			
			if(""==PRD_NO||null==PRD_NO){
				parent.showErrorMsg("请输入货品编号");
	            flag=false;
				return false;
			}
			if(""==PRD_NAME||null==PRD_NAME){
				parent.showErrorMsg("请输入货品名称");
	            flag=false;
				return false;
			}
	...
	function seachBoxPRD(key,id){
		...
		json[key]=val;
		$.ajax({ 
			'url':'advcorder.shtml?action=seachBox', //服务器的地址 
			'data':json, //参数 
...
//						html+="<a href='#' style='width:100px;' onmousedown=getPrdId("+data[i]["PRD_ID"]+","+id+")><li style='list-style-type: none;padding:0px 5px;float:left;'>"+data[i]["PRD_NO"]+"</li>"+
//							"<li style='list-style-type: none;padding:0px 5px;'>"+data[i]["PRD_NAME"]+"</li></a>";
						html+="<tr onmousedown=getPrdId('"+data[i]["PRD_ID"]+"',"+id+") onmouseover='mover(this)' onmouseout='mout(this)'>"+
								"<td style='border:solid  1px;'>"+data[i]["PRD_NO"]+"</td>"+
								"<td style='border:solid  1px;'>"+data[i]["PRD_NAME"]+"</td>"+
								"</tr>";
					}
					html+="</table>"
					$("#seachDiv").html(html);
		...
		var top=$("#"+key+id).offset().top;
		var left=$("#"+key+id).offset().left;
		var height=$("#"+key+id).height();
		$('#seachDiv').css({'left':left,'top':top+height});
		$('#seachDiv').show();
//		$("#"+key+"Td"+id).append('<div style="height:100px;width:100px;z-index: 99"></div>');
//		var val=$("#"+key+id).val();
//		var search="search"+key+id;
//		var input=key+id;
//		if(""==val||null==val){
//			return;
//		}
//		var json={};
//		json[key]=val;
//		var box=new seachBox(search,input);
//		box.ajax_request("advcorder.shtml?action=seachBox",json);
	}
	function closeDiv(){
		$('#seachDiv').hide();
	}
	function getPrdId(PRD_ID,id){
		$.ajax({ 
			'url':'advcorder.shtml?action=getPrdInfoById', //服务器的地址 
			'data':{"PRD_ID":PRD_ID}, //参数 
			'dataType':'text', //返回数据类型 
			'type':'POST', //请求类型 
			complete: function(xhr){
				eval("jsonResult = "+xhr.responseText);
	...
	function initPrd(rownum,data){
		$("#PRD_NO"+rownum).val(data["PRD_NO"]);
		$("#PRD_ID"+rownum).val(data["PRD_ID"]);
		$("#PRD_NAME"+rownum).val(data["PRD_NAME"]);
//		$("#PRD_TYPE"+rownum).val(data["PRD_TYPE"]);
		$("#PRD_SPEC"+rownum).val(data["PRD_SPEC"]);
		$("#PRD_COLOR"+rownum).val(data["PRD_COLOR"]);
		$("#BRAND"+rownum).val(data["BRAND"]);
		$("#STD_UNIT"+rownum).val(data["STD_UNIT"]);
		$("#PRICE"+rownum).val(data["FACT_PRICE"]);
		$("#ORDER_RECV_DATE"+rownum).val("");
		$("#SPECIAL_FLAG"+rownum).html("无");
		$("#SPCL_TECH_ID"+rownum).val("");
		$("#DECT_RATE"+rownum).val("1");
		$("#IS_FREEZE_FLAG"+rownum).prop("checked",false);
...
function closeInput(rownum){
	var PRD_NO=$("#PRD_NO"+rownum).val();
	var PRD_NAME=$("#PRD_NAME"+rownum).val();
	if(""!=PRD_NO||""!=PRD_NAME){
		$("#img"+rownum).hide();
	}else{
		$("#img"+rownum).show();

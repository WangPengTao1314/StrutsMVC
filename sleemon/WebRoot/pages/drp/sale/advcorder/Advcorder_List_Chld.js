...
//		}
	...
	$("#edit").click(function(){
		...
		$("#form1").attr("action","advcorder.shtml?action=toChildEdit");
		$("#form1").submit();
	});
...
function multiRecRemoveFreeze(){
	...
	var selRowId = parent.document.getElementById("selRowId").value;
	$.ajax({
		url: "advcorder.shtml?action=removeFreeze",
		type:"POST",
		dataType:"text",
		data:{"ADVC_ORDER_DTL_IDS":ids,"ADVC_ORDER_ID":selRowId},
		complete: function(xhr){
...
function multiCancel(){
	...
	var selRowId = parent.document.getElementById("selRowId").value;
	$.ajax({
		url: "advcorder.shtml?action=toCanCel",
		type:"POST",
		dataType:"text",
		data:{"ADVC_ORDER_DTL_IDS":ids,"ADVC_ORDER_ID":selRowId},
		complete: function(xhr){
...
function closeAdvc(){
	...
	$.ajax({
	 url: "advcorder.shtml?action=toClose&ADVC_ORDER_ID="+utf8(selRowId)+"&ADVC_ORDER_DTL_IDS="+utf8(ids),
	 type:"POST",
 	 dataType:"text",
	 complete: function(xhr){
		eval("jsonResult = "+xhr.responseText);

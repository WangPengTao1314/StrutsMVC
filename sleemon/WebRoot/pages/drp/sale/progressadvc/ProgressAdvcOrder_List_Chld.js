...
//		}
	...
	$("#edit").click(function(){
		...
		$("#form1").attr("action","propressadvcorder.shtml?action=toChildEdit");
		$("#form1").submit();
	});
...
function multiRecRemoveFreeze(){
	...
	var selRowId = parent.document.getElementById("selRowId").value;
	$.ajax({
		url: "propressadvcorder.shtml?action=removeFreeze",
		type:"POST",
		dataType:"text",
		data:{"ADVC_ORDER_DTL_IDS":ids,"ADVC_ORDER_ID":selRowId},
		complete: function(xhr){
...
function multiCancel(){
	...
	var selRowId = parent.document.getElementById("selRowId").value;
	$.ajax({
		url: "propressadvcorder.shtml?action=toCanCel",
		type:"POST",
		dataType:"text",
		data:{"ADVC_ORDER_DTL_IDS":ids,"ADVC_ORDER_ID":selRowId},
		complete: function(xhr){
...
function justHavePrd(){
	var trs = $("#ordertb tr:gt(0)");
	var flag = true ;
	trs.each(function(){
		 var PRD_ID = $(this).find("input[json='PRD_ID']").val();
		 if(null == PRD_ID || "" == $.trim(PRD_ID)){
			 flag = false;
		 }
	});
	return flag;

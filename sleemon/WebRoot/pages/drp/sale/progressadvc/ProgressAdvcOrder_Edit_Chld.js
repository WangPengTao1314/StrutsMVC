	...
	$.ajax({
		...
		complete: function (xhr){
			...
			if(jsonResult.success===true){
				saveSuccess("保存成功","propressadvcorder.shtml?action=toFrame");
			}else{
...
function childSave(){
	...
	$.ajax({
		url: "propressadvcorder.shtml?action=childEdit",
		type:"POST",
		dataType:"text",
...
//查找当前是否有选中的记录
	...
	$.ajax({
		url: "propressadvcorder.shtml?action=childDelete",
		type:"POST",
		dataType:"text",
...
function addRow(arrs){
	...
	$("#jsontb tr:last-child")
            ...
            .append('<td nowrap align="left"><input  json="PRICE" id="PRICE'+rownum+'" name="PRICE"  autocheck="true" label="单价" mustinput="true" size="5" type="text" onkeyup="countPRICE('+rownum+')" readonly   value="'+arrs[8]+'" /><input mustinput="true" inputtype="string" name="mustFlag"  style="width: 0px;"  type="text" >&nbsp;</td>')
            .append('<td nowrap align="left"><input  json="DECT_RATE" id="DECT_RATE'+rownum+'" name="DECT_RATE"  autocheck="true" label="折扣率" size="2" onkeyup="count('+rownum+')" type="text" readonly  value="'+arrs[9]+'" /><input mustinput="true" inputtype="string" name="mustFlag"  style="width: 0px;"  type="text" >&nbsp;</td>')
            .append('<td nowrap align="left"><input  json="DECT_PRICE" id="DECT_PRICE'+rownum+'" name="DECT_PRICE"  autocheck="true" label="折后单价" size="5"  type="text" onkeyup="countDECT_RATE('+rownum+')"  mustinput="true"   readonly   valueType="8,2"   value="'+arrs[10]+'" />&nbsp;</td>')
            .append('<td nowrap align="left"><input  json="ORDER_NUM" id="ORDER_NUM'+rownum+'" name="ORDER_NUM"  autocheck="true" label="订货数量" onkeyup="countORDER_NUM('+rownum+')" type="text" size="3" readonly   mustinput="true"        maxlength="22"  value="'+arrs[11]+'" /><input mustinput="true" inputtype="string" name="mustFlag"  style="width: 0px;"  type="text" >&nbsp;</td>')
            ...
            .append('<td nowrap align="left"><input  json="PAYABLE_AMOUNT" id="PAYABLE_AMOUNT'+rownum+'" name="PAYABLE_AMOUNT" readonly autocheck="true" label="应收金额"  type="text" size="7" onkeyup="countDECT_PRICE('+rownum+')"    value="'+arrs[12]+'" /><input mustinput="true" inputtype="string" name="mustFlag"  style="width: 0px;"  type="text" >&nbsp;</td>')
...
//货品通用选取
    function selcPrd(rownum){
    	...
    	var LEDGER_ID=$("#LEDGER_ID").val();
	     var obj=selCommon("BS_124", false, "PRD_ID"+rownum, "PRD_ID", "forms[0]","PRD_ID"+rownum+",PRD_NO"+rownum+",PRD_NAME"+rownum+",PRD_SPEC"+rownum+",PRD_COLOR"+rownum+",BRAND"+rownum+",STD_UNIT"+rownum+",PRICE"+rownum, 
...
function paddingPrice(){
	...
	checkBox.each(function(){
		...
		var DECT_PRICE=$(this).parent().parent().find("input[name='DECT_PRICE']").val();//折扣单价
		var DECT_RATE = 0;
		if(0==PRICE){
			DECT_RATE = 0
		}else{
			DECT_RATE = DECT_PRICE/PRICE;
			DECT_RATE = DECT_RATE.toFixed(2);
		}
		$(this).parent().parent().find("input[name='DECT_RATE']").val(DECT_RATE); 
	});
}

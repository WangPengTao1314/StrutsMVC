...
function allSave(){
	...
	$.ajax({
		url: "sersaleorder.shtml?action=edit",
		type:"POST",
		dataType:"text",
		...
		complete: function (xhr){
			...
			if(jsonResult.success===true){
				saveSuccess("保存成功","sersaleorder.shtml?action=toFrame");
			}else{
...
function childSave(){
	...
	$.ajax({
		url: "sersaleorder.shtml?action=childEdit",
		type:"POST",
		dataType:"text",
		data:{"childJsonData":jsonData,"SALE_ORDER_ID":selRowId},
		complete: function(xhr){
			eval("jsonResult = "+xhr.responseText);
			if(jsonResult.success===true){
...
//查找当前是否有选中的记录
	...
	$.ajax({
		url: "sersaleorder.shtml?action=childDelete",
		type:"POST",
		dataType:"text",
...
function gobacknew()
{
   newGoBack("sersaleorder.shtml?action=toFrame");
}
...
function addRow(arrs){
	...
	if(arrs[8]==null||arrs[8]==""||arrs[8]=="0"){
		...
		var method = "selectTechPage('"+rownum+"')";
		SPCL_TECH_FLAG="<font style='color: red'>有</font><input type='button' class='btn' value='查看' onclick="+method+" />"
	}
	
	$("#jsontb tr:last-child").after("<tr class='list_row"+classrow+"' onmouseover='mover(this)' onmouseout='mout(this)'></tr>");
	$("#jsontb tr:last-child")
		    .append("<td nowrap align='center'><input type='checkbox' style='height:12px;valign:middle' json='SALE_ORDER_DTL_ID' name='SALE_ORDER_DTL_ID"+rownum+"' value='"+arrs[0]+"'/></td>")
            ...
            .append('<td nowrap align="left"><input  json="DECT_PRICE" id="DECT_PRICE'+rownum+'" name="DECT_PRICE'+rownum+'"  autocheck="true" label="折后单价"  type="text"   inputtype="string"   readonly     maxlength="22"  value="'+arrs[9]+'"/>&nbsp;</td>')
            .append('<td nowrap align="left"><input  json="PRICE" id="PRICE'+rownum+'" name="PRICE'+rownum+'"  autocheck="true" label="单价"  type="text"   inputtype="string"     readonly   maxlength="22"  value="'+arrs[10]+'"/>&nbsp;</td>')
            .append('<td nowrap align="left"><input  json="OLD_PRICE" id="OLD_PRICE'+rownum+'" name="OLD_PRICE'+rownum+'"  autocheck="true" label="原价格"  type="text"   inputtype="string"     readonly   maxlength="22" value="'+arrs[11]+'" />&nbsp;</td>')
            .append('<td nowrap align="left"><input  json="ORDER_NUM" id="ORDER_NUM'+rownum+'" name="ORDER_NUM"  autocheck="true" label="订货数量"  type="text"  readonly inputtype="int" mustinput="true"  onkeyup="countTotal('+rownum+');"      maxlength="8"  value="'+arrs[12]+'"/>&nbsp;</td>')
            .append('<td nowrap align="left"><input  json="ORDER_AMOUNT" id="ORDER_AMOUNT'+rownum+'" name="ORDER_AMOUNT"  autocheck="true" label="订货金额"  type="text"   inputtype="string"   readonly     maxlength="22"  value="'+arrs[13]+'"/>&nbsp;</td>')
            .append('<td nowrap align="left"><input  json="REMARK" id="REMARK'+rownum+'" name="REMARK"  autocheck="true" label="备注"  type="text"   inputtype="string"       maxlength="250"  value="'+arrs[14]+'"/>&nbsp;</td>')

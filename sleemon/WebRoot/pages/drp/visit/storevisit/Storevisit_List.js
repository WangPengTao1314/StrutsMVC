...
$(function() {
	...
	//主从及主从从列表页面通用加载方法
	listPageInit("storevisit.shtml?action=toList");
    //新增和修改按钮初始化
	mtbAddModiInit();
	//新增和修改按钮初始化
	...
	$("#delete").click(function(){
 	...
 	$("#expertExcel").click(function(){
       var search = $("#search").val();
		$("#queryForm").attr("action","storevisit.shtml?action=expertExcel&module=" + getModule()+"&search="+search);
		$("#queryForm").submit();
	});
	//state();
	var selRowId = $("#selRowId").val();
 ...
 function toFlow(i) {
    ...
    document.affirm.sourceURI.value=ctxPath+"/storevisit.shtml?action=toList"+document.getElementById("paramUrl").value ;
	switch (Number(i)) {
	  case 1:
		affirmEvent(0);
		break;//提交
...
function listDelConfirm(){
	...
	var selRowId = $("#selRowId").val();
	 $.ajax({
	 	url: "storevisit.shtml?action=delete&STORE_VISIT_ID="+selRowId,
		type:"POST",
		complete: function(xhr){
			eval("jsonResult = "+xhr.responseText);
			if(jsonResult.success===true){

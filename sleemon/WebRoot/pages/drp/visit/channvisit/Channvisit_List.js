...
$(function() {
	...
	//主从及主从从列表页面通用加载方法
	listPageInit("channvisit.shtml?action=toList");
	//mtbAddModiInit();
	...
	$("#delete").click(function(){
	 ...
	 $("#expertExcel").click(function(){
       var search = $("#search").val();
		$("#queryForm").attr("action","channvisit.shtml?action=expertExcel&module=" + getModule()+"&search="+search);
		$("#queryForm").submit();
	 });
 ...
 function toFlow(i) {
    ...
    document.affirm.sourceURI.value=ctxPath+"/channvisit.shtml?action=toList"+document.getElementById("paramUrl").value ;
	switch (Number(i)) {
	  case 1:
		affirmEvent(0);
		break;//提交
...
function listDelConfirm(){
	...
	var selRowId = $("#selRowId").val();
	 $.ajax({
	 	url: "channvisit.shtml?action=delete&CHANN_VISIT_ID="+selRowId,
		type:"POST",
		complete: function(xhr){
			eval("jsonResult = "+xhr.responseText);
			if(jsonResult.success===true){

...
$(function() {
	...
	//主从及主从从列表页面通用加载方法
	listPageInit("expandvisit.shtml?action=toList");
	...
	$("#add").click(function(){
		...
		parent.window.gotoBottomPage("toEditFrame");
	});
	...
	$("#delete").click(function(){
	 ...
	 $("#expertExcel").click(function(){
		...
		$("#queryForm").attr("action","expandvisit.shtml?action=expertExcel&module=" + getModule()+"&search="+search);
		$("#queryForm").submit();
	});
		var selRowId = $("#selRowId").val();
	    if(null == selRowId || "" == selRowId){
 ...
 function toFlow(i) {
    ...
    document.affirm.sourceURI.value=ctxPath+"/expandvisit.shtml?action=toList"+document.getElementById("paramUrl").value ;
	switch (Number(i)) {
	  case 1:
		affirmEvent(0);
		break;//提交
...
function listDelConfirm(){
	...
	var selRowId = $("#selRowId").val();
	 $.ajax({
	 	url: "expandvisit.shtml?action=delete&EXPAND_VISIT_ID="+selRowId,
		type:"POST",
		complete: function(xhr){
			eval("jsonResult = "+xhr.responseText);
			if(jsonResult.success===true){

...
//查询预定单列表
 function queryOrderNo(){
    ...
    var tempParams = "";
    if("销售退款" == BILL_TYPE){
    	tempParams = " and STATE in ('审核通过') and BILL_TYPE='终端销售' ";
    }else{
 	...
 	$("#SALE_DATE").val("");
 	$("#ORDER_RECV_DATE").val("");
	var returnArray = selCommon('BS_186', false, 'ADVC_ORDER_ID', 'ADVC_ORDER_ID', 'forms[0]',
		'ADVC_ORDER_ID,ADVC_ORDER_NO,TERM_ID,TERM_NO,TERM_NAME,SALE_DATE,SALE_PSON_NAME,CUST_NAME,TEL,ORDER_RECV_DATE,ADVC_AMOUNT,PAYABLE_AMOUNT,PAYED_TOTAL_AMOUNT,PAYED_TOTAL_AMOUNT_TEMP',
		'ADVC_ORDER_ID,ADVC_ORDER_NO,TERM_ID,TERM_NO,TERM_NAME,SALE_DATE,SALE_PSON_NAME,CUST_NAME,TEL,ORDER_RECV_DATE,ADVC_AMOUNT,PAYABLE_AMOUNT,PAYED_TOTAL_AMOUNT,PAYED_TOTAL_AMOUNT_TEMP','con');

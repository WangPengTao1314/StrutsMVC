	...
	$("#save").click(function(){
		...
		if("list" == actionType){
			...
			if(allPayAmont != parseFloat(parent.topcontent.document.getElementById("STATEMENTS_AMOUNT").value)){
				parent.showErrorMsg("'本次退款金额'和'付款金额'相加的总额不等,请重新填写！");
				return;
			}
			allSave();
		}
...
function allSave(){
	...
	$.ajax({
		url: "custrefundments.shtml?action=edit",
		type:"POST",
		dataType:"text",
		...
		complete: function (xhr){
			...
			if(jsonResult.success===true){
				saveSuccess("保存成功","custrefundments.shtml?action=toFrame");
...
function childSave(){
	...
	$.ajax({
		url: "custrefundments.shtml?action=childEdit",
		type:"POST",
		dataType:"text",
...
//查找当前是否有选中的记录
	...
	$.ajax({
		url: "custrefundments.shtml?action=childDelete",
		type:"POST",
		dataType:"text",
...
function gobacknew()
{
   newGoBack("custrefundments.shtml?action=toFrame");
}

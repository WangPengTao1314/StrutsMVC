...
$(function(){
	...
	$("#edit").click(function(){
		...
		$("#PRMT_PRD_GROUP_IDS").val(ids);
		$("#form1").attr("action","prmtplan.shtml?action=toChildEdit");
		$("#form1").submit();
	});
...
function setSelOperateEx() {
	...
	if(vstate){
		...
		if (state == "\u63d0\u4ea4"||state == "\u9000\u56de"||state == "\u5ba1\u6838\u901a\u8fc7"||state=="已发布"||state=="已终止") {
			//btnDisable(["delete","edit"]);
			btnHidden(["delete","edit"]);
		}
	}
}

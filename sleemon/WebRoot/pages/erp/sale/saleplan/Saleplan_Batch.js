...
function url(){
	var url="saleplan.shtml?action=toBatchList";
...
function saveBatchT(){
	 ...
	 for(var i=0;i<eid.length;i++){
	    if(eid[i].checked==true){
	       t++;
	       var SALE_PLAN_ID = topcontent.document.getElementById("SALE_PLAN_ID"+(i+1)).value;
	       SALE_PLAN_IDs += SALE_PLAN_ID+",";
	 ...
	 SALE_PLAN_IDs     = SALE_PLAN_IDs.substr(0,SALE_PLAN_IDs.length-1);
     ...
     if(t!=0) {
	     $.ajax({
	       ...
	       parent.showErrorMsg("请选择渠道销售指标信息");
	 	   return;
	   }
}

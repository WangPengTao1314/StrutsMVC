...
//返回
function gobacknew(){
   newGoBack("turnoverhd.shtml?action=toFrame");
}
...
function save(){
	...
	$.ajax({
		url: "turnoverhd.shtml?action=edit",
		type:"POST",
		dataType:"text",
...
//				saveSuccess(utf8Decode(jsonResult.messages),"turnoverhd.shtml?action=toFrame");
				saveConfirm(utf8Decode(jsonResult.messages),"goFrame('turnoverhd.shtml?action=toFrame');");
			}else{

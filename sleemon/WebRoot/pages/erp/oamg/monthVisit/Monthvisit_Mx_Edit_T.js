...
function addRow(arrs){
	 ...
	 $("#jsontb tr:last-child")
	        .append("<td nowrap align='center'>"
		    			+"<input type='checkbox' style='height:12px;valign:middle' json='MONTH_VISIT_PLAN_DTL_ID' id='MONTH_VISIT_PLAN_DTL_ID"+rownum+"' name='MONTH_VISIT_PLAN_DTL_ID"+rownum+"' value='"+arrs[0]+"'/></td>")
...
function getNumType(param){
	...
	var PLAN_VISIT_NUM = parent.bottomcontent.document.getElementsByName("PLAN_VISIT_NUM");
	for(var i=1;i<PLAN_VISIT_NUM.length+1;i++){
	   if(parent.bottomcontent.document.getElementById("PLAN_VISIT_NUM"+i)!=null){
	   var str1 = parent.bottomcontent.document.getElementById("PLAN_VISIT_NUM"+i).value;
       if(!r.test(str1)){
          parent.showErrorMsg("拜访次数请输入正整数");
	      return ;

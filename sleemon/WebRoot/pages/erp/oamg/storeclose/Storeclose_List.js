...
$(function(){
	...
	//主从及主从从列表页面通用加载方法
	listPageInit("storeclose.shtml?action=toList&module=" + module);
	//新增和修改按钮初始化
	mtbAddModiInit();
	//删除监听.参数1：删除action，参数2：删除主键Id
	mtbDelListener("storeclose.shtml?action=delete", "SPCL_STORE_CC_REQ_ID");
    ...
    $("#expertExcel").click(function(){
        ...
        var selRowId = parent.document.getElementById("selRowId").value;
		$("#queryForm").attr("action","storeclose.shtml?action=expertExcel&module=" + getModule()+"&search="+search+"&PCL_STORE_CC_REQ_ID="+selRowId);
		$("#queryForm").submit();  
	});
 ...
 function toFlow(i) {
    ...
    document.affirm.sourceURI.value=ctxPath+"/storeclose.shtml?action=toList"+document.getElementById("paramUrl").value ;
	switch (Number(i)) {
	  case 1:
		affirmEvent(0);
		break;//提交
 ...
 function toAudit(){
     var selRowId = $("#selRowId").val();
     $.ajax({
		url: "storeclose.shtml?action=toAudit&SPCL_STORE_CC_REQ_ID="+selRowId,
		type:"POST",
		dataType:"text",

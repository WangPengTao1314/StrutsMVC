...
<%@ page import="com.hoperun.commons.edit.*"%>
<jsp:useBean id="date" scope="page" class="com.hoperun.commons.edit.TimeStamp"/>
...
public void InitUpload(String realpath){
	...
	uw.getInstance();
	UploadBean bean = uw.InitPara(sStyleName);
	try{
		...
		String sUploadFilePath=sUploadDir;//"/UploadFile/";
		...
		//sUploadDir =sUploadFilePath +calendar.get(Calendar.YEAR) + "/"+ (calendar.get(Calendar.MONTH)+1) + "/" + calendar.get(Calendar.DAY_OF_MONTH) + "/";
  ...
  //DoSave();
   ...
   //设置上传文件类型
	...
	// Retreive the current file
		com.hoperun.commons.edit.File myFile = up.getFiles().getFile(i);
		if (!myFile.isMissing()) {
			...
			System.out.println("--1--"+sPathFileName);
		}
	}
	System.out.println("--2--"+sSaveFileName+"--2--"+sOriginalFileName+"--2--"+sPathFileName);
    out.println("<script language=javascript>");

...
$(function () {
	//页面初始化
	listPageInit("pdtmenu.shtml?action=toList");
	//新增和修改按钮初始化
	mtbAddModiInit();
	//删除监听.参数1：删除action，参数2：删除主键Id
	mtbDelListener("pdtmenu.shtml?action=delete", "PRD_ID");
...
function listDelConfirm(){
	...
	var selRowId = $("#selRowId").val();
	 $.ajax({
	 	url: "pdtmenu.shtml?action=delete&PRD_ID="+selRowId,
		type:"POST",
...
function listStopConfirm(){
	...
	var selRowId = $("#selRowId").val();
	 $.ajax({
		url: "pdtmenu.shtml?action=changeStateStop&PRD_ID="+utf8(selRowId),
		type:"POST",
		dataType:"text",
		complete: function(xhr){
			eval("jsonResult = "+xhr.responseText);
...
function listStartConfirm(){
	...
	var selRowId = $("#selRowId").val();
    $.ajax({
	 url: "pdtmenu.shtml?action=changeStateStart&PRD_ID="+utf8(selRowId),
	 type:"POST",
 	 dataType:"text",
	 complete: function(xhr){
		eval("jsonResult = "+xhr.responseText);

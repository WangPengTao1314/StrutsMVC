...
$(function () {
	//页面初始化
	listPageInit("provider.shtml?action=toList");
	//新增和修改按钮初始化
	mtbAddModiInit();
...
function listDelConfirm(){
	...
	var selRowId = $("#selRowId").val();
	 $.ajax({
	 	url: "provider.shtml?action=delete&PRVD_ID="+selRowId,
		type:"POST",
...
function listStopConfirm(){
	...
	var selRowId = $("#selRowId").val();
	 $.ajax({
		url: "provider.shtml?action=changeStateStop&PRVD_ID="+utf8(selRowId),
		type:"POST",
		dataType:"text",
		complete: function(xhr){
			eval("jsonResult = "+xhr.responseText);
...
function listStartConfirm(){
	...
	var selRowId = $("#selRowId").val();
    $.ajax({
	 url: "provider.shtml?action=changeStateStart&PRVD_ID="+utf8(selRowId),
	 type:"POST",
 	 dataType:"text",
	 complete: function(xhr){
		eval("jsonResult = "+xhr.responseText);

...
$(function () {
	...
	listPageInit("distributor.shtml?action=toList");
	//新增和修改按钮初始化
	mtbAddModiInit();
	...
	$("#expdata").click(function(){
		 $("#queryForm").attr("action","distributor.shtml?action=toExpData&module=" + module);
		 $("#queryForm").submit();
	 });
	// 停用
	$("#stop").click(function(){
...
function listDelConfirm(){
	...
	var selRowId = $("#selRowId").val();
	 $.ajax({
	 	url: "distributor.shtml?action=delete&DISTRIBUTOR_ID="+selRowId,
		type:"POST",
		complete: function(xhr){
			eval("jsonResult = "+xhr.responseText);
			if(jsonResult.success===true){
				parent.showMsgPanel("删除成功");
				window.location="distributor.shtml?action=toList&DISTRIBUTOR_ID="+selRowId;
			}else{
...
function listStopConfirm(){
	...
	var selRowId = $("#selRowId").val();
	 $.ajax({
		url: "distributor.shtml?action=changeStateTy&DISTRIBUTOR_ID="+utf8(selRowId),
		type:"POST",
		dataType:"text",
		complete: function(xhr){
			eval("jsonResult = "+xhr.responseText);
...
function listStartConfirm(){
	...
	var selRowId = $("#selRowId").val();
    $.ajax({
	 url: "distributor.shtml?action=changeStateQy&DISTRIBUTOR_ID="+utf8(selRowId),
	 type:"POST",
 	 dataType:"text",
	 complete: function(xhr){
		eval("jsonResult = "+xhr.responseText);

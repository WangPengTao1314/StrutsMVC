...
$(function(){
	...
	$("#save").click(function(){
		...
		$.ajax({
			url: "shipPoint.shtml?action=edit",
			type:"POST",
			dataType:"text",
			data:{"jsonData":jsonData,"SHIP_POINT_ID":selId},
			complete: function(xhr){
				eval("jsonResult = "+xhr.responseText);
				if(jsonResult.success===true){
					parent.showMsgPanel("保存成功");
					var SHIP_POINT_ID = utf8Decode(jsonResult.messages);
					//上帧页面跳转至编辑后的记录
					window.parent.topcontent.location="shipPoint.shtml?action=toList"+parent.window.reqParamsEx();

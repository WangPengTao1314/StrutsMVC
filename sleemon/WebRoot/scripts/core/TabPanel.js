...
TabPanel.prototype = {
  ...
  addTab : function(tabitem){
    ...
    {
	  alert('您已经打开了'+this.maxLength+'个选项卡，请关闭不用的选项卡。');
      return false;
    }
    ...
    else if(this.scrollFinish)
    {
      ...
      if(tabitem.icon) {
        ...
        if(title.width()>(this.tabWidth-26-wFix)) {
         //add by zzb 2014-3-4
          var textL = title.text().length;
          if(textL==8){
        	  tab.css("width","120px");
        	  title.width("110px"); 
          }else if(textL>=9){
        	var w = this.tabWidth+16-wFix;
        	tab.css("width","150px");
        	title.width(w); 
          }else{
        	  title.width((this.tabWidth-30-wFix));//old
          }
  ...
  show : function(position, notExecuteMoveSee){
    ...
    if(this.scrollFinish)
    {
      ...
      else
      {
        ...
        //this.tabpanel_mover.find('.aw').removeClass('aw');
        ...
        for(var i = 0; i < this.tabs.length; i++){
			if(i != position){
				var objTmp = this.tabs[i].tab.find('.closer');
				objTmp.removeClass('closer');
				objTmp.addClass('aw');
			}else{
				var objTmp2 = this.tabs[i].tab.find('.aw');
				objTmp2.removeClass('aw');
				objTmp2.addClass('closer');
			}
		}
  ...
  setTitleColor : function(position){
	for(var i=0; i<this.tabs.length; i++){
		if(i != position){
			this.tabs[i].title.css('color','#5e5e5e');
			this.tabs[i].title.css('font-weight','bold');
			//this.tabs[i].css('background','url(../images/main/gray_close.jpg) no-repeat');
		}else{
			this.tabs[i].title.css('color','#000');
		}
	}
  }
};
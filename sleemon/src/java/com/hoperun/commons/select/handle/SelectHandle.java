/**
  ...
  *@author zhuxw      
*/
package com.hoperun.commons.select.handle;
...
import com.hoperun.commons.select.object.TableObject;

// TODO: Auto-generated Javadoc
/**
 * The Class SelectHandle.
 * 
 * @module 系统模块
 * @func 系统基础类
 * @version 1.1
...
public class SelectHandle {
	 ...
	 */
	public SelectHandle(String fileName) throws Exception {
		//从hashtable中取文档对象
	 ...
	 * @return TableObject
	 */
	public TableObject getCurrentTableObject(String id, boolean defaultSelectFlag) {
		...
		aTableObject.setIsShowSearchLayer(getAttributeValue(tableElem, "isShowSearchLayer").equals("T"));
		aTableObject.setIsTree(getAttributeValue(tableElem, "isTree").equals("T"));
		aTableObject.setIsWLXX(getAttributeValue(tableElem, "isWLXX").equals("T"));
		aTableObject.setIsWLDWXX(getAttributeValue(tableElem, "isWLDWXX").equals("T"));
		...
		boolean[] isChildCon = new boolean[fields.size()];
		String[] childTabId = new String[fields.size()];
		...
		for (int i = 0; i < fields.size(); i++) {
			...
			isChildCon[i] = getAttributeValue(elem, "isChildCon").equals("T");
			childTabId[i] = getAttributeValue(elem, "childTabId");
		}
		...
		aTableObject.setIsChildCon(isChildCon);
		aTableObject.setChildTabId(childTabId);
		return aTableObject;
	}
	 ...
	 * @return TableObject[]
	 */
	public TableObject[] getSubTableObjects(String pid) {
		Element tableElem = doc.getRootElement();
		...
		TableObject[] arrTableObject = new TableObject[list.size()];
